...
String indexlink;
String Nefry_Web::beginWeb(String _viwePage) {
	if (_viwePage.equals("index")) {
		String content = F(
		...
		content += F("</p>");
		return createHtml(F("Nefry Menu"), "", content);
	}
	else if (_viwePage.equals("nefry_css")) {
		return	F(
	...
	else if (_viwePage.equals("NotFound")) {
		return createHtml(F("NotFound Page"), "", F("<h1>Nefry NotFound Page</h1><p>お探しのページは見つかりませんでした。<br>下のリンクからトップページに戻ることができます。</p><a href=\"/\">Back to top</a>"));
	}
	else if (_viwePage.equals("Reset")) {
		return  createHtml(F("Nefry Reset"), "",F("<h1>Nefry Reset</h1><p>Reset start!</p><a href=\"/\">Back to top</a>"));
	}
	else if (_viwePage.equals("jsform")) {
		return F("function jsSubmit(formN){formN.submit();}");
	}
	else if (_viwePage.equals("onreset")) {
...
/*
void Nefry_Web::setupWebCaptivePortal(void) {
